...
if make_plot:
    ...
    pars[2] *= scale * dist_scale
    p.ioff()
    p.errorbar(phys_radius, data["Vt"], yerr=data['eVt'],
               fmt='D', color='b')
    p.plot(phys_radius, vcirc(phys_radius, *pars), 'r-')
...
if make_rotmodel:
    ...
    # Convert rmax to pc
    pars[2] *= scale * dist_scale * 1000.

    # Put into m/s.
    smooth_model = (vcirc(radii, *pars) * 1000.) * -np.cos(pas) * \
    ...
    new_header["COMMENT"] = "Smooth rotation model of DISKFIT output. Uses Eq.5" \
        " from Meidt et al. 2008."
